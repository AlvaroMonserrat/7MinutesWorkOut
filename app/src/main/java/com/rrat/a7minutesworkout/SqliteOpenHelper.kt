...
class SqliteOpenHelper(context: Context, factory: SQLiteDatabase.CursorFactory?) : SQLiteOpenHelper(context, DATABASE_NAME, factory, DATABASE_VERSION) {
    ...
    override fun onCreate(db: SQLiteDatabase?) {
        // CREATE TABLE history(_id, INTEGER PRIMARY KEY, complete_date, TEXT)
    ...
    fun getAllCompletedDatesList(): List<String>{
        val completedDatesList = ArrayList<String>()

        val db = this.readableDatabase
        val cursor = db.rawQuery("SELECT * FROM $TABLE_HISTORY", null)

        while (cursor.moveToNext()){
            val dataValue = (cursor.getString(cursor.getColumnIndex(COLUMN_COMPLETED_DATE)))
            completedDatesList.add(dataValue)

        }
        cursor.close()

        return completedDatesList
